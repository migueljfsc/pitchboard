...
import { boardPath, readBoardId, readShareSlug } from "./routes";
...
describe("readShareSlug", () => {
  ...
  it("rejects the wrong length or the wrong alphabet", () => {
    expect(readShareSlug("/s/3g839hk")).toBeNull();
    expect(readShareSlug("/s/aeiou123")).toBeNull();
    expect(readShareSlug("/s/3g839hk77")).toBeNull();
  });
